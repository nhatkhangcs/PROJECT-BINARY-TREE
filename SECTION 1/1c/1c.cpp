...
bool multiOutputError(string str){
  for(int i=0;i<str.length();i++){
    ...
    // if(isOperator(str[i])){
    //   if(op1=='/'&&op2=='*'
    //     &&(nxtOperatorIndex(str,i)<nxtParenIndex(str,i)
    //     &&nxtParenIndex(str,i) != -1))
    //   {
...
bool syntaxError(string str){
    ...
    for(int i=0;i<str.length();i++){
      if(isOperator(str[i]) && (i==0||i==str.length()-1)) flag = true;
    }
